...
static UIView *PHFindSubviewOfClass(UIView *root, Class cls) {
    ...
    for (UIView *sub in root.subviews) { UIView *found = PHFindSubviewOfClass(sub, cls); if (found) return found; }
    return nil;
}
static UIButton *PHFindButton(UIView *root, NSString *title) {
    ...
    for (UIView *sub in root.subviews) { UIButton *found = PHFindButton(sub, title); if (found) return found; }
    return nil;
}
static UILabel *PHFindContentLabel(UIScrollView *scroll) {
    if (!scroll) return nil;
    for (UIView *sub in scroll.subviews) if ([sub isKindOfClass:UILabel.class]) return (UILabel *)sub;
    return nil;
}
...
static void PHSetJSONContent(id self, NSString *selector) {
    ...
    UIView *view = [self valueForKey:@"view"]; UIView *panel = view.subviews.firstObject; if (!panel) return;
    UIScrollView *scroll = (UIScrollView *)PHFindSubviewOfClass(panel, UIScrollView.class); UILabel *content = PHFindContentLabel(scroll);
    if (content) { content.text = json; content.font = [UIFont monospacedSystemFontOfSize:12.5 weight:UIFontWeightRegular]; }
}
static void PHConfigureHierarchyButton(id self) {
    UIView *view = [self valueForKey:@"view"]; UIButton *button = PHFindButton(view, @"Voltar"); if (!button) return;
    [button setTitle:@"JSON" forState:UIControlStateNormal]; [button removeTarget:nil action:NULL forControlEvents:UIControlEventTouchUpInside]; [button addTarget:self action:@selector(ph_jsonTapped) forControlEvents:UIControlEventTouchUpInside];
}
static void PHConfigureJSONScreen(id self) {
    UIView *view = [self valueForKey:@"view"]; UIView *panel = view.subviews.firstObject; if (!panel) return;
    UILabel *subtitle = nil; for (UIView *sub in panel.subviews) if ([sub isKindOfClass:UILabel.class] && [((UILabel *)sub).text isEqualToString:@"Elemento Web selecionado"]) { subtitle=(UILabel *)sub; break; }
    if (subtitle) subtitle.text = @"Filtro JSON";
    WKWebView *webView = nil; @try { webView=[self valueForKey:@"highlightedWebView"]; } @catch (__unused NSException *e) {}
    if (![webView isKindOfClass:WKWebView.class]) { @try { webView=(WKWebView *)PHFindSubviewOfClass(view, WKWebView.class); } @catch (__unused NSException *e) {} }
    if (![webView isKindOfClass:WKWebView.class]) { PHSetJSONContent(self, @""); return; }
    [webView evaluateJavaScript:PHSelectedElementSelectorJS() completionHandler:^(id result, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{ PHSetJSONContent(self, (!error && [result isKindOfClass:NSString.class]) ? result : @""); });
    }];
    UIButton *left = PHFindButton(panel, @"Hierarquia"); if (!left) left=PHFindButton(panel, @"Voltar");
...
@interface PHV22JSONBridge : NSObject @end
@implementation PHV22JSONBridge
- (void)ph_jsonTapped { PHSetJSONMode(self,YES); ((void (*)(id, SEL, BOOL))objc_msgSend)(self,@selector(render:),YES); }
+ (void)load {
    Class target=NSClassFromString(@"PHInspectorViewController"); if (!target) return;
    Method render=class_getInstanceMethod(target,@selector(render:)); if(render){PHOriginalRender=method_getImplementation(render);method_setImplementation(render,(IMP)PHRenderHook);}
    Method copy=class_getInstanceMethod(target,@selector(copyTapped)); if(copy){PHOriginalCopy=method_getImplementation(copy);method_setImplementation(copy,(IMP)PHCopyHook);}
    Method back=class_getInstanceMethod(target,@selector(backTapped)); if(back){PHOriginalBack=method_getImplementation(back);method_setImplementation(back,(IMP)PHBackHook);}
    Method details=class_getInstanceMethod(target,@selector(showInspectorDetails:subtitle:)); if(details){PHOriginalDetails=method_getImplementation(details);method_setImplementation(details,(IMP)PHDetailsHook);}
    Method jsonTap=class_getInstanceMethod(self,@selector(ph_jsonTapped)); if(jsonTap) class_addMethod(target,@selector(ph_jsonTapped),method_getImplementation(jsonTap),method_getTypeEncoding(jsonTap));
}
@end
