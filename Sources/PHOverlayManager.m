...
- (void)showSelectionPrompt {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UILabel *bubble = [self labelWithText:@"🔍  Toque no elemento que deseja inspecionar"
...
- (void)showSelectedView:(UIView *)view {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UILabel *title = [self labelWithText:@"ProjetoH Inspector"
                                        font:[UIFont boldSystemFontOfSize:21.0]
                                       color:UIColor.whiteColor];
        UILabel *subtitle = [self labelWithText:@"Elemento selecionado"
                                           font:[UIFont systemFontOfSize:14.0]
                                          color:[UIColor colorWithWhite:0.72 alpha:1.0]];

        NSString *text = [PHInspectorViewController descriptionForView:view];
        UILabel *details = [self labelWithText:text
                                          font:[UIFont monospacedSystemFontOfSize:13.0 weight:UIFontWeightRegular]
        ...
        [NSLayoutConstraint activateConstraints:@[
            ...
            [details.topAnchor constraintEqualToAnchor:subtitle.bottomAnchor constant:20.0],
            [details.leadingAnchor constraintEqualToAnchor:panel.leadingAnchor constant:22.0],
            [details.trailingAnchor constraintEqualToAnchor:panel.trailingAnchor constant:-22.0],
...
- (void)closeTapped {
    [[PHOverlayManager sharedManager] dismissOverlay];
}
...
- (UIWindow *)activeKeyWindow {
    for (UIScene *candidate in UIApplication.sharedApplication.connectedScenes) {
        ...
        for (UIWindow *window in scene.windows.reverseObjectEnumerator) {
            if (!window.hidden && window.alpha > 0.0 && window.rootViewController != nil) return window;
        }
    }
    return nil;
}
...
- (void)processInspectionEvent:(UIEvent *)event {
    ...
    UIView *selected = hit;
    if (selected == window && window.rootViewController.view != nil) {
        selected = window.rootViewController.view;
    }
