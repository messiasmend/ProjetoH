...
/* ProjetoH V21 — generic. Keep WebFrame's proven selection/hide flow intact. */
...
static NSString *PH21Selector(NSDictionary*f){id s=f[@"selector"];if([s isKindOfClass:NSString.class]&&[s length])return s;NSDictionary*a=[f[@"action"] isKindOfClass:NSDictionary.class]?f[@"action"]:nil;s=a[@"selector"];return [s isKindOfClass:NSString.class]&&[s length]?s:nil;}
...
static NSString *PH21Name(NSDictionary*m){NSString*t=[m[@"tag"] isKindOfClass:NSString.class]?[m[@"tag"] lowercaseString]:@"";if([t isEqualToString:@"lottie-player"])return @"Lottie Player";NSString*v=[m[@"id"] isKindOfClass:NSString.class]?m[@"id"]:nil;return v.length?[NSString stringWithFormat:@"#%@",v]:t.length?[t uppercaseString]:@"Elemento Web";}
...
/* Capture only for metadata. The actual hide operation remains WebFrame's original implementation. */
...
/* Convert only the URL scope of filters created by WebFrame. This preserves its
   selector/action and all other JSON fields, while making the rule global. */
