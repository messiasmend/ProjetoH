...
# WebFrame custom-filters.json: native top-level array, Documents preferred.
...
methods='''- (void)savePendingFilters {
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"Salvar" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action){
        ...
        for(NSString *selector in PHPendingSelectors.copy){
            BOOL exists=NO;
            for(id filter in filters){NSString *existing=PHSelectorFromFilter(filter);if([existing isEqualToString:selector]){exists=YES;break;}}
...
- (void)showHiddenElements {
    ...
    for(NSString *selector in valid){
        NSString *name=PHDisplayNameForSelector(selector);
        [alert addAction:[UIAlertAction actionWithTitle:[NSString stringWithFormat:@"Reativar: %@",name] style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action){
...
# Closing the inspector cancels only unsaved preview changes.
...
# Fix artifacts produced by the older V12 GUI generator.
s=s.replace('s.componentsSeparatedByString:@" "][0]','[s componentsSeparatedByString:@" "].firstObject')
s=s.replace('Reativar: %@\n%@','Reativar: %@ — %@')
s=s.replace('@"…"','@"..."')
# If an older generated dictionary contains escaped quotes, normalize only dictionary literals.
s=s.replace('@{\\"trigger\\":','@{@"trigger":')
s=s.replace('\\"url-filter\\":\\".*\\"','@"url-filter":@".*"')
s=s.replace('\\"action\\":@{','@"action":@{')
s=s.replace('\\"type\\":\\"css-display-none\\"','@"type":@"css-display-none"')
s=s.replace('\\"selector\\":s','@"selector":s')
s=s.replace('\\"selector\\":selector','@"selector":selector')
